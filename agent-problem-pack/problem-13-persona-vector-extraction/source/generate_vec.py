...
def get_persona_effective(pos_path, neg_path, trait, threshold=50):
    ...
    # Keep only examples where the positive prompt actually elicited the trait
    # and the negative prompt suppressed it, while both stayed coherent.
    mask = (
...
def save_persona_vector(model_name, pos_path, neg_path, trait, save_dir, threshold=50):
    ...
    # The persona vector at each layer is the mean activation under positive
    # prompts minus the mean activation under negative prompts.
